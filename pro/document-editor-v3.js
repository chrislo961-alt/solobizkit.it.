...
function isDocumentForm(){return Boolean(modalBody?.querySelector('.invoice-lines,.estimate-lines'));}
function cleanupNonDocument(){
  modalForm?.classList.remove('doc-editor-v3');
  modalBody?.querySelector('.doc-editor-summary')?.remove();
  modalBody?.querySelector('.doc-editor-section-title')?.remove();
  modalBody?.querySelector('.doc-editor-tip')?.remove();
  const eyebrow=modalTitle?.closest('.modal-head')?.querySelector('.eyebrow');
  if(eyebrow&&eyebrow.textContent==='DOCUMENT EDITOR V3')eyebrow.textContent='SOLOBIZKIT PRO';
}
...
function updateLineAmounts(){if(!isDocumentForm())return;const currency=currentCurrency();rows().forEach(row=>{addLineAmount(row);const amount=row.querySelector('.doc-line-amount');if(amount)amount.textContent=money(lineAmount(row),currency);});renumber();updateSummary();}
...
function enhance(){
  if(!modal?.open||!modalBody?.children.length)return;
  if(!isDocumentForm()){cleanupNonDocument();return;}
  modalForm?.classList.add('doc-editor-v3');
  installSummary();
  installLineSection();
  updateLineAmounts();
  modalTitle?.closest('.modal-head')?.querySelector('.eyebrow')?.replaceChildren(document.createTextNode('DOCUMENT EDITOR V3'));
}
modalBody?.addEventListener('input',event=>{if(isDocumentForm()&&event.target.matches('input,select,textarea'))requestAnimationFrame(updateLineAmounts);});
modalBody?.addEventListener('change',event=>{if(isDocumentForm()&&event.target.matches('select'))requestAnimationFrame(updateLineAmounts);});
modal?.addEventListener('close',cleanupNonDocument);

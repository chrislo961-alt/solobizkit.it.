...
function flash(message, tone = 'success') {
  ...
  if (!node) { node = document.createElement('div'); node.id = 'documentUxToast'; node.className = 'doc-ux-toast'; document.body.appendChild(node); }
  node.className = `doc-ux-toast ${tone}`; node.textContent = message; node.hidden = false;
  clearTimeout(node._hideTimer); node._hideTimer = setTimeout(() => { node.hidden = true; }, 2600);
}

async function workspace(force = false) {
  session ||= await getSession();
  if (!session?.user?.id) throw new Error('Sign in to continue.');
  if (force || !workspaceCache) workspaceCache = await loadWorkspace(session.user.id);
  return workspaceCache;
}
async function requireWrite() {
  const data = await workspace();
  if (!data.workspace?.canWrite) throw new Error('This workspace role is read only.');
  return data;
}
...
async function duplicateLatest(type) {
  try {
    ...
    if (!latest) return flash(`No ${type}s to reuse yet.`, 'error');
    if (type === 'invoice') await duplicateInvoice(latest.id); else await duplicateEstimate(latest.id);
  } catch (error) { console.error(error); flash(error?.message || 'Could not reuse the latest document.', 'error'); }
}

function invoiceStatus(invoice) { return normalizeInvoiceStatus(invoice); }
function renderStatusGuide(type, items) {
  document.querySelector('.doc-ux-status-guide')?.remove();
  const toolbar = document.querySelector('.content .card .toolbar');
  if (!toolbar) return;
  const steps = type === 'invoice' ? [['draft','Draft'],['sent','Sent'],['overdue','Overdue'],['paid','Paid']] : [['draft','Draft'],['sent','Sent'],['accepted','Accepted'],['converted','Converted'],['declined','Declined']];
...
function addReuseButton(type, canWrite) {
  const toolbar = document.querySelector('.content .card .toolbar');
  if (!toolbar) return;
  toolbar.querySelector('[data-duplicate-latest]')?.remove();
  if (!canWrite) return;
  const button = document.createElement('button');
  button.type = 'button'; button.className = 'btn secondary doc-ux-reuse'; button.dataset.duplicateLatest = type; button.textContent = '↻ New from last';
  const primary = toolbar.querySelector('.btn.primary'); if (primary) toolbar.insertBefore(button, primary); else toolbar.appendChild(button);
}

function addInvoiceDuplicateButtons(canWrite) {
  document.querySelectorAll('[data-duplicate-invoice]').forEach((node)=>{ if(!canWrite) node.remove(); });
  if (!canWrite) return;
  document.querySelectorAll('[data-edit-invoice]').forEach((edit) => {
    const cell = edit.parentElement; if (!cell || cell.querySelector(`[data-duplicate-invoice="${CSS.escape(edit.dataset.editInvoice)}"]`)) return;
    const button = document.createElement('button'); button.type = 'button'; button.className = 'mini-btn'; button.dataset.duplicateInvoice = edit.dataset.editInvoice; button.textContent = 'Duplicate'; cell.insertBefore(button, edit.nextSibling);
  });
}
function addEstimateDuplicateButtons(canWrite) {
  document.querySelectorAll('[data-duplicate-estimate]').forEach((node)=>{ if(!canWrite) node.remove(); });
  if (!canWrite) return;
  document.querySelectorAll('.estimate-actions [data-edit]').forEach((edit) => {
    const actions = edit.closest('.estimate-actions'); if (!actions || actions.querySelector(`[data-duplicate-estimate="${CSS.escape(edit.dataset.edit)}"]`)) return;
    const button = document.createElement('button'); button.type = 'button'; button.className = 'mini-btn'; button.dataset.duplicateEstimate = edit.dataset.edit; button.textContent = 'Duplicate'; actions.insertBefore(button, edit.nextSibling);
  });
}
...
async function enhance() {
  ...
  try {
    ...
    addReuseButton(type, canWrite);
    if (type === 'invoice') addInvoiceDuplicateButtons(canWrite); else addEstimateDuplicateButtons(canWrite);
    renderStatusGuide(type, type === 'invoice' ? data.invoices : data.estimates);
  } catch (error) { console.error('Could not load document UX', error); }
}

document.addEventListener('click', (event) => {
  const button = event.target.closest('button'); if (!button) return;
  if (button.dataset.duplicateInvoice) { event.preventDefault(); event.stopImmediatePropagation(); duplicateInvoice(button.dataset.duplicateInvoice); return; }
  if (button.dataset.duplicateEstimate) { event.preventDefault(); event.stopImmediatePropagation(); duplicateEstimate(button.dataset.duplicateEstimate); return; }
  if (button.dataset.duplicateLatest) { event.preventDefault(); duplicateLatest(button.dataset.duplicateLatest); return; }
  if (button.hasAttribute('data-doc-filter')) {
    const select = document.querySelector(pageType === 'estimate' ? '#estimateStatus' : '#invoiceStatus'); if (!select) return;
    select.value = button.dataset.docFilter || ''; select.dispatchEvent(new Event('change', { bubbles: true }));

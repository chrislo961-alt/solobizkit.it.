...
async function saveProfile(){
  ...
  const payload={user_id:session.user.id,customer_id:String(form.get('customerId')),name:String(form.get('name')||'').trim(),active:editing?.active??true,frequency:String(form.get('frequency')||'monthly'),interval_count:Math.max(1,Number(form.get('intervalCount')||1)),next_issue_date:String(form.get('nextIssueDate')||today()),payment_terms_days:Math.max(0,Number(form.get('paymentTermsDays')||14)),currency:String(form.get('currency')||'USD'),language:'en',discount_rate:Math.max(0,Math.min(100,Number(form.get('discountRate')||0))),notes:String(form.get('notes')||'').trim()||null,country_code:null,vat_mode:'standard',updated_at:new Date().toISOString()};

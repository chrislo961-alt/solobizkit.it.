...
async function ensureContext(){
  ...
  if(!scheduleDays){
    const {data,error}=await supabase.from('company_settings').select('reminder_schedule_days').eq('user_id',dataOwner).maybeSingle();
    if(error)throw error;
    scheduleDays=Array.isArray(data?.reminder_schedule_days)?data.reminder_schedule_days:[0,7,14];
  }
}

export async function scheduleInvoiceReminders(invoiceId,dueDate){
  await ensureContext();
  if(!workspace?.canWrite)throw new Error('Editor access is required to schedule reminders.');
  if(!dueDate)return;
  const schedule=(scheduleDays||[]).map(Number).filter((n)=>Number.isFinite(n)&&n>=0&&n<=365);
  const {error:deleteError}=await supabase.from('reminder_queue').delete().eq('invoice_id',invoiceId).eq('user_id',dataOwner).eq('status','ready');
  if(deleteError)throw deleteError;
  if(!schedule.length)return;
  const rows=[...new Set(schedule)].sort((a,b)=>a-b).map((day)=>({user_id:dataOwner,invoice_id:invoiceId,reminder_day:day,due_on:addDays(dueDate,day),status:'ready'}));
...
async function fetchInvoice(id){
  await ensureContext();
  const {data,error}=await supabase.from('invoices').select('id,invoice_number,status,due_date,customer_id').eq('id',id).eq('user_id',dataOwner).maybeSingle();
  if(error)throw error;if(!data)throw new Error('Invoice not found.');
  const customer=data.customer_id?await supabase.from('customers').select('name,email').eq('id',data.customer_id).eq('user_id',dataOwner).maybeSingle():{data:null,error:null};
  if(customer.error)throw customer.error;return{invoice:data,customer:customer.data};
}

async function sendReminder(id){
  await ensureContext();
  if(!workspace?.canWrite)throw new Error('Editor access is required to send reminders.');
  ...
  const {data,error}=await supabase.functions.invoke('send-invoice-message',{body:{invoiceId:id,kind:'reminder',recipient:customer.email,subject,html,product:'solobizkit'}});
...
window.addEventListener('solobizkit:workspace-updated',()=>{workspace=null;dataOwner=null;scheduleDays=null;injectButtons();});

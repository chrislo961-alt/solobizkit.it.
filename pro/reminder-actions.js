...
async function ensureContext(){
  ...
  if(!scheduleDays){
    const {data,error}=await supabase.from('company_settings').select('reminder_schedule_days').eq('user_id',session.user.id).maybeSingle();
...
(async()=>{try{session=await getSession();if(session?.user?.id)await ensureContext();}catch(error){console.error('Reminder actions init failed',error);}})();

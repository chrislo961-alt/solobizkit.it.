...
function buildEvents({ customer, invoices, estimates, deliveries }) {
  ...
  for (const invoice of invoices) {
    events.push({ date: invoice.created_at || invoice.issue_date, type: 'invoice', title: `Invoice ${invoice.invoice_number}`, meta: `${money(invoice.total, invoice.currency)} · due ${invoice.due_date || '—'}`, status: invoice.status, link: '/pro/?view=invoices' });
    if (invoice.sent_date) events.push({ date: invoice.sent_date, type: 'sent', title: 'Invoice sent', meta: invoice.invoice_number, status: 'sent', link: '/pro/?view=invoices' });
    if (invoice.paid_date) events.push({ date: invoice.paid_date, type: 'paid', title: 'Payment recorded', meta: `${invoice.invoice_number} · ${money(invoice.total, invoice.currency)}`, status: 'paid', link: '/pro/?view=invoices' });
  }

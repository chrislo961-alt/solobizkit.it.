...
function currencySummary(items, fallback = 'USD') {
  const totals = new Map();
  for (const item of items) {
    const currency = String(item.currency || fallback || 'USD').toUpperCase();
    totals.set(currency, (totals.get(currency) || 0) + Number(item.total || 0));
  }
  if (!totals.size) return money(0, fallback);
...
function buildEvents({ customer, invoices, estimates, deliveries }) {
  ...
  for (const estimate of estimates) {
    const link = `/pro/estimates/?estimate=${encodeURIComponent(estimate.id)}`;
    events.push({ date: estimate.created_at || estimate.issue_date, type: 'estimate', title: `Estimate ${estimate.estimate_number}`, meta: `${money(estimate.total, estimate.currency)} · valid until ${estimate.valid_until || '—'}`, status: estimate.status, link });
    if (estimate.responded_at) events.push({ date: estimate.responded_at, type: 'response', title: `Estimate ${estimate.status}`, meta: `${estimate.estimate_number} · ${money(estimate.total, estimate.currency)}`, status: estimate.status, link });
    if (estimate.converted_invoice_id) events.push({ date: estimate.updated_at, type: 'converted', title: 'Estimate converted to invoice', meta: estimate.estimate_number, status: 'converted', link: `/pro/?view=invoices&invoice=${encodeURIComponent(estimate.converted_invoice_id)}` });
  }
  for (const invoice of invoices) {
    const link = `/pro/?view=invoices&invoice=${encodeURIComponent(invoice.id)}`;
    events.push({ date: invoice.created_at || invoice.issue_date, type: 'invoice', title: `Invoice ${invoice.invoice_number}`, meta: `${money(invoice.total, invoice.currency)} · due ${invoice.due_date || '—'}`, status: invoice.status, link });
    if (invoice.sent_date) events.push({ date: invoice.sent_date, type: 'sent', title: 'Invoice sent', meta: invoice.invoice_number, status: 'sent', link });
    if (invoice.paid_date) events.push({ date: invoice.paid_date, type: 'paid', title: 'Payment recorded', meta: `${invoice.invoice_number} · ${money(invoice.total, invoice.currency)}`, status: 'paid', link });
  }
  for (const delivery of deliveries) events.push({ date: delivery.sent_at || delivery.created_at, type: 'email', title: delivery.kind === 'estimate' ? 'Estimate email' : delivery.kind === 'reminder' ? 'Payment reminder' : 'Invoice email', meta: `${delivery.recipient || 'Recipient'}${delivery.subject ? ` · ${delivery.subject}` : ''}`, status: delivery.status, link: delivery.invoice_id ? `/pro/?view=invoices&invoice=${encodeURIComponent(delivery.invoice_id)}` : delivery.estimate_id ? `/pro/estimates/?estimate=${encodeURIComponent(delivery.estimate_id)}` : '' });
...
function addHistoryButtons() {
  document.querySelectorAll('[data-customer-actions][data-customer-id]').forEach((cell) => {
    const customerId = cell.dataset.customerId;
    if (!customerId || cell.querySelector(`[data-history-customer="${CSS.escape(customerId)}"]`)) return;
    const button = document.createElement('button');
    button.type = 'button';
    button.className = 'mini-btn';
    button.dataset.historyCustomer = customerId;
    button.textContent = 'History';
    cell.prepend(button, document.createTextNode(' '));
  });
}
...
const observer = new MutationObserver(() => {
  if (queued) return;
  queued = true;
  requestAnimationFrame(() => { queued = false; addHistoryButtons(); });
});
observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['data-customer-actions', 'data-customer-id'] });
addHistoryButtons();
